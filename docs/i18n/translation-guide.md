...
- **禁止語 specimen 保留雙語**：當某段是「絕對不要說出的具體語句」清單（must-not-say blocklist，例如 `You're absolutely right!`、`Thanks for…` 這類表演性附和），保留英文原句「並」附上中文對照，兩者並列。理由：雙語運作的 agent 兩種語言都可能反射性說出，只留一種語言會漏掉另一種反射。此為例外——「應該說」的生成式範例（good response、範例對話）仍只譯為使用者語言，不需並列英文
- 修正明顯不自然、容易造成誤讀的直譯
